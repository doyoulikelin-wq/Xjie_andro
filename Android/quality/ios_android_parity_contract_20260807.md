...
- Scrollable UI assertions for asynchronously produced content must wait for that exact semantics node (or an explicit loaded-state tag) before revealing or asserting it; a static shell title is not readiness evidence. Dialog dismissal must be observed through an explicit state tag before the owning close action is used; an immediate system-back race is not acceptable evidence.
...
- Main-based feature branch `codex/android-ios-parity-20260807` was first pushed as `f9f581f2cec1cf44a2fae0a5524d58cd13e81e8d`; PR [#1](https://github.com/doyoulikelin-wq/Xjie_andro/pull/1) targets `main`. Its first exact-head run [31175685594](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31175685594) concluded **failure** at the pre-test emulator boundary. KVM repair commit `43157811daf5db8c9ad4b59ae916234a04e21b76` then triggered [31179377876](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31179377876): backend/build passed and accelerated UI executed, but the clean-checkout origin mismatch failed closed. Explicit-origin repair `5d12df5cfd90e89036cee3eeb683563860e82e98` triggered [31181786778](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31181786778): backend/build, KVM, origin, local interception and `13/14` standard tests passed, while one asynchronous semantics race failed and compact/large stopped fail-closed. Its shared readiness repair is locally green; the replacement exact-head run, merge SHA and post-merge `andro/main` CI do not yet exist and must not be inferred.
