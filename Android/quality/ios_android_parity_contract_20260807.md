...
| K | Release configuration, signed artifact, hosted CI, GitHub main delivery | Gradle/manifest/quality/GitHub | `ReleaseConfigContractTest`, `ReleaseArtifactVerifierContractTest`, `test_ci_requires_kvm_before_emulator_and_keeps_evidence_upload_fail_closed`, `test_ci_pins_deterministic_origin_without_local_properties_dependency` | local fail-closed guards implemented and fixture-tested; run `31175685594` exposed missing hosted KVM permissions, then run `31179377876` proved KVM/boot/evidence repair but exposed a clean-checkout dependency on untracked `local.properties`; the UI job now pins the exact deterministic origin without weakening `418`/zero-escape; replacement exact-head run and merge/main CI pending; real Release remains blocked on explicit production URL, external signing and a verified candidate package |
