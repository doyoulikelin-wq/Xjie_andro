...
- Network-backed text assertions must wait for exact text, retain the merged-semantics lookup, then scroll and assert visibility. Off-screen lazy targets first wait for a persistent readiness tag on an always-composed non-lazy owner, then await and use the caller-provided scroll root before revealing the exact target tag and asserting it is displayed. Readiness must be scoped to the owning screen state, must not leak into child editors, add a lazy item, shift list indices, or replace the scroll root tag; a static shell/root title is not readiness evidence. Dialog dismissal must be observed through an explicit state tag before the owning close action is used; an immediate system-back race is not acceptable evidence.
...
| K | Release configuration, signed artifact, hosted CI, GitHub main delivery | Gradle/manifest/quality/GitHub | `ReleaseConfigContractTest`, `ReleaseArtifactVerifierContractTest`, `test_ci_requires_kvm_before_emulator_and_keeps_evidence_upload_fail_closed`, `test_ci_pins_deterministic_origin_without_local_properties_dependency`, `test_connected_tests_share_real_app_factory_profile_gate_and_runtime_ledger` | local fail-closed guards implemented and fixture-tested; runs `31175685594`, `31179377876`, and `31181786778` respectively exposed KVM admission, clean-checkout origin ownership, and one cold-run semantics race. Candidate `02560b0e2b27799e0c28cb804ec73413075782b1` triggered cancelled run `31184274631` before qualification. Hardened head `8263cefbaabfbc5cc7b3b1b0bfb38bf9934591fa` retains merged text semantics, uses screen-scoped persistent non-lazy readiness owners, awaits and uses the caller-provided root, preserves product list indices/root tags, and rejects direct bypass/lazy sentinels. Exact-head run `31188223697` passed backend `392/392`, Python `44/44`, JVM `232/232`, build/lint, KVM, and UI `14/14` in all three profiles; downloaded artifacts and zero-unknown runtime ledger were inspected. This evidence-only follow-up still needs exact-head CI, then merge/main CI. Real Release remains blocked on explicit production URL, external signing and a verified candidate package |
...
- Exact Android quality-tool Python inventory and execution: `44/44`, with zero failure, error, skip or expected failure.
...
- The replacement exact-head run `31179377876` proved that KVM admission, accelerated boot, test launch and fail-closed evidence upload were repaired: all `14` standard tests ran and the artifact was preserved. It also exposed the next clean-checkout contract bug: without ignored `Android/local.properties`, Gradle selected `http://10.0.2.2:8000`, so the exact production-origin interceptor correctly returned `418` to all network-backed tests (`13` failures; local-original device test passed). The UI job now owns one explicit `API_BASE_URL_DEBUG=https://www.jianjieaitech.com/api`; a named regression proves the old workflow fails, pins job-level ownership with no override/local-file dependency, and preserves exact origin, unknown-request and zero-escape contracts.
- Exact-head run `31181786778` proved the KVM and explicit-origin repairs: backend `392/392`, Python `44/44`, JVM `232/232`, assemble/lint, accelerated boot, local interception, and all `14` standard executions reached the assertions. It failed only because the Meals/Profile test used a static page title as a readiness proxy and called `performScrollTo()` before the network-backed empty-state semantics existed (`13` pass, `1` fail; compact/large stopped fail-closed).
- Initial repair `02560b0e2b27799e0c28cb804ec73413075782b1` added exact-text waiting, but review proved its final unmerged lookup weakened accessibility-facing evidence and the same-class scan found canonical Weight plus Medication plans/reactions inside lazy containers. Run `31184274631` was therefore cancelled after backend/build succeeded. `waitForAndScrollToText` now retains merged lookup; `waitForAndScrollToTag` waits persistent XAGE/Medication readiness tags on non-lazy owners, then awaits and uses the supplied scroll root before exact lazy-target reveal. Medication readiness exists only on the loaded dashboard (`editor == null`). The named contract rejects the weaker candidate, ignored/hard-coded roots, direct bypass, duplicate/comment-only or editor-leaking readiness, and lazy sentinel items while preserving list indices and scroll-root tags. The focused tests passed Meals/Profile `3/3` plus Weight/Medication `2/2`, and final local gates passed Python `44/44`, JVM `232/232`, build/lint, and UI `42/42`.
- Hardened behavior head `8263cefbaabfbc5cc7b3b1b0bfb38bf9934591fa` then passed exact-head run [31188223697](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31188223697): backend `392/392`, Python `44/44`, JVM `232/232`, assemble/lint, KVM admission, and standard/compact/large-text UI `14/14` each. Downloaded `android-ui-profile-results` artifact `8998461179` has digest `sha256:433ba7dca434a238ec992bf701487d2068f903a7076fd285ebd746d74f78ced5`; its run status is exit `0` at `large_text_api35`, the tracked inventory validator passes, and logs/runtime teardown contain no localhost origin, HTTP `418`, unknown deterministic request, empty ledger, or escaped-request marker. The evidence-only follow-up commit must independently pass exact-head CI before readiness/merge.
...
- Main-based feature branch `codex/android-ios-parity-20260807` was first pushed as `f9f581f2cec1cf44a2fae0a5524d58cd13e81e8d`; PR [#1](https://github.com/doyoulikelin-wq/Xjie_andro/pull/1) targets `main`. Runs [31175685594](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31175685594), [31179377876](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31179377876), and [31181786778](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31181786778) are the three preserved fail-closed incidents. Initial semantics repair `02560b0e2b27799e0c28cb804ec73413075782b1` triggered deliberately cancelled [run 31184274631](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31184274631). Hardened behavior head `8263cefbaabfbc5cc7b3b1b0bfb38bf9934591fa` is qualified by successful [run 31188223697](https://github.com/doyoulikelin-wq/Xjie_andro/actions/runs/31188223697). The evidence-head, merge SHA, and post-merge `andro/main` CI do not yet exist and must not be inferred.
...
- Therefore the Android software replication is locally verified and behavior head `8263cef` has exact-head CI success. Evidence-head exact CI, merge/main delivery, and a signed production Android release are not yet claimed by this snapshot.
