...
| K | Release configuration, signed artifact, hosted CI, GitHub main delivery | Gradle/manifest/quality/GitHub | `ReleaseConfigContractTest`, `ReleaseArtifactVerifierContractTest`, `test_ci_requires_kvm_before_emulator_and_keeps_evidence_upload_fail_closed`, `test_ci_pins_deterministic_origin_without_local_properties_dependency`, `test_connected_tests_share_real_app_factory_profile_gate_and_runtime_ledger` | local fail-closed guards implemented and fixture-tested; runs `31175685594`, `31179377876`, and `31181786778` respectively exposed KVM admission, clean-checkout origin ownership, and one cold-run semantics race. Candidate `02560b0e2b27799e0c28cb804ec73413075782b1` triggered `31184274631`; backend/build passed, but the run was cancelled before qualification after review found an unmerged-semantics weakening and Weight/Medication lazy siblings. The hardened helpers retain merged text semantics, use screen-scoped persistent non-lazy readiness owners for lazy targets, await and use the caller-provided root, preserve product list indices/root tags, reject direct bypass and lazy sentinels, and passed Meals/Profile `3/3`, Weight/Medication `2/2`, Python `44/44`, JVM `232/232`, build/lint, and local UI `42/42`; replacement exact-head and merge/main CI are pending. Real Release remains blocked on explicit production URL, external signing and a verified candidate package |
