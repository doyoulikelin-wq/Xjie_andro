...
| K | Release configuration, signed artifact, hosted CI, GitHub main delivery | Gradle/manifest/quality/GitHub | `ReleaseConfigContractTest`, `ReleaseArtifactVerifierContractTest` | local fail-closed guards implemented and fixture-tested; GitHub PR/CI/merge identity pending; real Release remains blocked on explicit production URL, external signing and a verified candidate package |
...
- Exact Android quality-tool Python inventory and execution: `42/42`, with zero failure, error, skip or expected failure.
...
- Local source is on main-based feature branch `codex/android-ios-parity-20260807`, based on Android HEAD `261ad12bf4c31b10a26d0b13fdbb2844cbba4232`; a final feature commit has not yet been created. Feature SHA, PR, hosted exact-head CI, merge SHA and post-merge `andro/main` verification are mandatory pending delivery fields and must not be inferred from local results.
...
- Therefore the Android software replication is locally verified, but neither GitHub delivery nor a signed production Android release is claimed by this snapshot.
